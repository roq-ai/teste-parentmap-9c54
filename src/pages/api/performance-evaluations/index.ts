...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getPerformanceEvaluations() {
    ...
    const response = await prisma.performance_evaluation
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'performance_evaluation'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createPerformanceEvaluation() {
    ...
    const data = await prisma.performance_evaluation.create({
      data: body,
    });
